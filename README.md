...
81 variables, 6 outputs. Every variable has a description and a default in `00-variables.tf`.
...
```bash
# 1. Clone
git clone https://github.com/heyvaldemar/amazon-ec2-nextcloud-docker-swarm-pipeline-terraform
cd amazon-ec2-nextcloud-docker-swarm-pipeline-terraform
...
| `nextcloud_secret_1_arn` | `arn:aws:secretsmanager:YOUR-REGION-X:000000000000:secret:nextcloud-1-000000` | The ARN of the AWS Secrets Manager secret that contains the credentials for the Nextcloud |
| `route53_zone_1_name` | `example.com` | Domain name for the ACM certificate |
| `acm_1_certificate_1_domain_name` | `nextcloud.example.com` | Domain name for the ACM certificate |
| `nextcloud_external_url` | `https://nextcloud.example.com` | URL on which Nextcloud will be reachable |
| `nextcloud_trusted_domain` | `nextcloud.example.com` | Domain on which Nextcloud will be reachable |
...
Every release from v1.1.5 on carries three files made on GitHub's runner with a short-lived identity and no stored key: `amazon-ec2-nextcloud-docker-swarm-pipeline-terraform-<tag>.tar.gz`, a `git archive` of exactly the tree the tag points at; `amazon-ec2-nextcloud-docker-swarm-pipeline-terraform-<tag>.tar.gz.sigstore.json`, a keyless [Sigstore](https://www.sigstore.dev/) signature over it; and `amazon-ec2-nextcloud-docker-swarm-pipeline-terraform-<tag>.intoto.jsonl`, [SLSA](https://slsa.dev/) build provenance from the SLSA generator. To check them with nothing from this repository trusted:

```bash
cosign verify-blob amazon-ec2-nextcloud-docker-swarm-pipeline-terraform-<tag>.tar.gz \
  --bundle amazon-ec2-nextcloud-docker-swarm-pipeline-terraform-<tag>.tar.gz.sigstore.json \
  --certificate-identity-regexp '^https://github.com/heyvaldemar/amazon-ec2-nextcloud-docker-swarm-pipeline-terraform/' \
  --certificate-oidc-issuer https://token.actions.githubusercontent.com

slsa-verifier verify-artifact amazon-ec2-nextcloud-docker-swarm-pipeline-terraform-<tag>.tar.gz \
  --provenance-path amazon-ec2-nextcloud-docker-swarm-pipeline-terraform-<tag>.intoto.jsonl \
  --source-uri github.com/heyvaldemar/amazon-ec2-nextcloud-docker-swarm-pipeline-terraform
```
...
The [Terraform Verification](https://github.com/heyvaldemar/amazon-ec2-nextcloud-docker-swarm-pipeline-terraform/actions/workflows/terraform-verification.yml?query=branch%3Amain) workflow runs on every push, pull request, and weekly: `terraform fmt -check`, `terraform init -lockfile=readonly`, `terraform validate`, `tflint`, and actionlint on the workflow itself.
